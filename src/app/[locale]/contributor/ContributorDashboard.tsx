...
export function ContributorDashboard({ contributor }: Props) {
  ...
  const tDashboard = useTranslations('Dashboard.contributor')
  ...
  return (
    <main className="mx-auto max-w-[68rem] px-6 lg:px-8 py-12 pt-[var(--header-height)]">
      <nav className="mb-8 text-sm text-muted-foreground">
        ...
        <span className="text-foreground">{tDashboard('breadcrumb')}</span>
      </nav>

      <h1 className="mb-8 text-3xl font-bold tracking-tight">{tDashboard('heading', { name: contributor.displayName })}</h1>

      <div className="grid gap-6 sm:grid-cols-2">
        <div className="rounded-xl border bg-card p-6 shadow-sm">
          <h2 className="mb-2 text-sm font-medium text-muted-foreground">{tDashboard('reputation')}</h2>
          <span className="text-4xl font-bold tabular-nums">{contributor.reputationPoints}</span>
        </div>

        <div className="rounded-xl border bg-card p-6 shadow-sm">
          <h2 className="mb-2 text-sm font-medium text-muted-foreground">{tDashboard('email')}</h2>
