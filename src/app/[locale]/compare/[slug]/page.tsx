...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const [rowA, rowB] = await Promise.all([
    db.select().from(Products).where(and(eq(Products.slug, parsed.slugA), inArray(Products.status, ['published', 'delisted']))).limit(1),
    db.select().from(Products).where(and(eq(Products.slug, parsed.slugB), inArray(Products.status, ['published', 'delisted']))).limit(1),
  ])
