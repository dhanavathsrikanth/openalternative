...
const alertVariants = cva(
  "relative w-full rounded-radius-card border p-4 text-body-sm [&>svg+div]:translate-y-[-3px] [&>svg]:absolute [&>svg]:left-4 [&>svg]:top-4 [&>svg]:text-text-tertiary [&>svg~*]:pl-7",
  {
    variants: {
      variant: {
...
>(({ className, ...props }, ref) => (
  <div
    ref={ref}
    className={cn("text-body-sm text-text-tertiary [&_p]:leading-relaxed", className)}
    {...props}
  />
))
